...
6. flight_per_person: ціна авіа НА ОДНУ особу. 
   - Якщо вказано "вартість авіа 410 євро", це значить flight_per_person = 410.
   - Якщо вказано загальну суму за всіх — розділи її на (дорослих + дітей).
7. hotel_prices: список ЗАГАЛЬНИХ цін за проживання для кожного готелю (в тому ж порядку, що в тексті).
8. hotel_stars: зірковість кожного готелю (0, 3, 4, 5).
9. other_per_person: інші витрати на особу.

ФОРМАТ: Тільки JSON.
КРИТИЧНО: Будь дуже уважним до вартості авіа. Не ігноруй її.
"""
...
async def format_tour_message(user_text: str, do_cleanup: bool = False) -> str:
    ...
    if price_data and price_data.get("hotel_prices") and price_data.get("flight_per_person") is not None:
        ...
        for idx, hotel_total in enumerate(hotel_prices):
            ...
            # 2. Markup (Margin)
            # Reverting exactly to what you asked: +150 for < 350, otherwise * 1.43
            if base_cost < 350:
                final = base_cost + 150
            else:
            ...
            logger.info(f"CALC: Hotel={matched_hotels[idx]['hotel']}, PriceIn={hotel_total}, Flight={flight}, Tax={tax}, Base={base_cost}, Final={final}")
