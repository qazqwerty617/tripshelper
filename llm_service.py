...
Твоє завдання: виправити помилки розпізнавання (галлюцинації Whisper, повтори, незв'язні символи), щоб текст став чистим, АЛЕ КРИТИЧНО ВАЖЛИВО ЗБЕРЕГТИ КОЖНЕ СЛОВО, що стосується готелів, цін, дат та часу.

ПРАВИЛА:
1. Виправляй лише явну "кашу" в словах (наприклад "Тенер і Фе" -> "Тенерифе", "Коста Бра во" -> "Коста-Брава").
2. Зберігай назви готелів як є, навіть якщо вони звучать незвично або написані кирилицею.
3. НЕ ВИДАЛЯЙ цифри, ціни, валюту (€, євро, дол), дати та ЧАС перельоту.
4. Якщо в тексті є повторювані фрази (баг Whisper), видали повтори.
5. НЕ ДОДАВАЙ нічого від себе, не намагайся зробити текст "красивим" або "літературним". 
6. Поверни ТІЛЬКИ виправлений текст. Якщо текст вже нормальний — поверни його без змін.
"""
...
(ОБОВ'ЯЗКОВО порожній рядок між готелями!)
...
КРИТИЧНІ ПРАВИЛА:
1. ВІДСТУПИ: Між кожним готелем у списку "варіанти проживання" МАЄ БУТИ один порожній рядок. Це критично для читабельності.
2. ЖОДНИХ ГАЛЮЦИНАЦІЙ: Використовуй ТІЛЬКИ ті готелі, які надані в блоці "ЗНАЙДЕНІ В БАЗІ ГОТЕЛІ". Не замінюй їх на інші, навіть якщо вони здаються тобі "кращими" або "схожими". Якщо в базі вказано "BJ Playamar", ти пишеш "BJ Playamar", а не "BG Hotel Caballero".
3. КІЛЬКІСТЬ: Якщо в базі 8 готелів — у фінальному тексті має бути РІВНО 8 готелів. Не пропускай жодного.
4. ПОРЯДОК: Готелі в списку "варіанти проживання" та ціни в рядку 💰 МАЮТЬ відповідати один одному за номером.
5. ЦІНИ: ЗАБОРОНЕНО вигадувати чи перераховувати ціни. Використовуй ТІЛЬКИ готові значення з блоку "РОЗРАХОВАНІ ЦІНИ".
6. ЗІРКИ: Став зірки (3★, 4★, 5★) ЗАВЖДИ, якщо вони є в базі поруч із назвою готелю.
7. ХАРЧУВАННЯ: Для кожного готелю обов'язково вказуй харчування після значка 🥑. 
   - Використовуй тільки: "сніданки", "сніданки + вечері", "повний пансіон", "все включено", "ультра все включено", "без харчування".
8. МОВА: Весь текст УКРАЇНСЬКОЮ, назви готелів — АНГЛІЙСЬКОЮ (як у базі).
9. ОДНАКОВІСТЬ: Кожне твоє повідомлення має виглядати ідентично за структурою.
...
2. Якщо менеджер вказав тип харчування один раз для всіх готелів (наприклад, "всі готелі на сніданках" або "харчування всюди все включено"), то поверни цей тип харчування для КОЖНОГО готелю у списку.
3. Використовуй ТІЛЬКИ ці назви:
   ...
   - "без харчування" (RO)
4. КРИТИЧНО: Якщо в тексті "напівпансіон" або "напівпансін" — ЗАВЖДИ пиши "сніданки + вечері".
5. Якщо тип харчування не вказано для конкретного готелю, пиши "не вказано".
6. ФОРМАТ: Тільки JSON {"meals": ["тип 1", "тип 2"]}.
"""
...
async def transcribe_voice(file_bytes: bytes) -> str:
    ...
    if not active_keys:
        ...
        for _ in range(len(active_keys)):
            ...
            whisper_prompt = "Турагент диктує підбірку готелів та ціни. Напрямки: Тенерифе, Майорка, Коста-Брава, Крит, Родос, Корфу, Кіпр. Готелі: Rixos, Mitsis, Grecotel, Iberostar, Sol, Melia. Типи харчування: сніданки, вечері, напівпансіон, все включено. Ціна в євро, авіапереліт, трансфер, страховка."
            
            files = {"file": ("voice.ogg", file_bytes, "audio/ogg")}
            data = {
                "model": "whisper-large-v3",
                "prompt": whisper_prompt,
                # Видаляємо жорстку мову "uk", щоб Whisper краще розпізнавав суржик та змішану мову
            }
    ...
    if OPENROUTER_API_KEY:
        try:
            ...
            files = {"file": ("voice.ogg", file_bytes, "audio/ogg")}
            # OpenRouter models: openai/whisper-large-v3 or groq/whisper-large-v3
            data_or = {
                "model": "openai/whisper-large-v3",
                "prompt": "Турагент диктує підбірку готелів та ціни: Тенерифе, Майорка, Коста-Брава, Крит, Родос, Корфу, Кіпр, сніданки, вечері, все включено, євро."
            }
