...
Твоє завдання: виправити помилки розпізнавання, щоб текст став зрозумілим, АЛЕ ЗБЕРЕГТИ КОЖНЕ СЛОВО, що стосується готелів та цін.

ПРАВИЛА:
- Виправляй лише явну "кашу" в словах (наприклад "Тенер і Фе" -> "Тенерифе", "Коста Бра во" -> "Коста-Брава").
- Зберігай назви готелів як є, навіть якщо вони звучать незвично (це можуть бути іспанські, грецькі чи турецькі назви).
- НЕ ВИДАЛЯЙ цифри та ціни.
...
4. ХАРЧУВАННЯ: Для кожного готелю обов'язково вказуй харчування після значка 🥑. 
   - Якщо в блоці "ТИПИ ХАРЧУВАННЯ" вказано "напівпансіон" — ЗАМІНЮЙ на "сніданок + вечеря".
   - Якщо вказано "все включено", "сніданки" тощо — пиши як є.
...
2. Використовуй ТІЛЬКИ ці назви (або їх еквіваленти):
   - "сніданки" (BB)
   - "сніданок + вечеря" (якщо в тексті "напівпансіон", "HB", "half board", "напівпансін")
   - "сніданок + обід + вечеря" (FB)
   - "все включено" (All Inclusive, AI)
   - "ультра все включено" (UAI)
   - "без харчування" (RO)
3. КРИТИЧНО: Якщо в тексті "напівпансіон" або "напівпансін" — ЗАВЖДИ пиши "сніданок + вечеря".
