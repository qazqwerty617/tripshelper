...
2. НЕ ПИШИ список готелів (варіанти проживання) — я додам його сам.
3. НЕ ПИШИ ціни — я додам їх сам.
4. НЕ ПИШИ фразу "Ціна актуальна..." — я додам її сам.
"""
...
async def format_tour_message(user_text: str, do_cleanup: bool = False) -> str:
    ...
    if result:
        ...
        # Split LLM result into Intro and Recommendations
        # Usually LLM will put Recommendations after a few lines of Intro
        parts = result.split("\n\n")
        intro = "\n\n".join(parts[:4]) # Heuristic for intro
        recommendations = "\n\n".join(parts[4:]) # Remaining is recommendations
        
        final_message = f"{intro}\n{options_block}{footer_block}{recommendations}"
