...
- КРИТИЧНО ВАЖЛИВО: Ти маєш знайти і повернути ВСІ готелі, які перелічив менеджер. Не пропускай жодного!
...
ПРАВИЛО ДЛЯ ЦІНИ АВІА:
- "X євро квиток", "X євро білет", "авіа X€", "квитки по X€" — це ЗАВЖДИ ціна ЗА ОСОБУ → flight_per_person = X
- "X євро на всіх", "загальна вартість квитків X€" → flight_per_person = X / (adults + children)

ПРАВИЛО ДЛЯ ЦІНИ ГОТЕЛЮ:
- Ціна готелю — це ЗАВЖДИ загальна вартість номера (для всіх людей), НЕ на особу.
- Повертай саме таку загальну ціну. Ділити на людей НЕ ПОТРІБНО — це зробить програма.

ПРАВИЛО ДЛЯ ЗІРОК ГОТЕЛЮ:
- Якщо вказано "3 зірки", "3*", "3★" → 3. Якщо "4 зірки" → 4. Якщо невідомо → 0.
"""
...
Твоє завдання: перетворити вхідний текст-чернетку (голосове або текстове повідомлення з містом вильоту, курортом, готелем, харчуванням та цінами) у вірний шаблон.
...
def _inject_links(text: str, hotel_link_map: dict) -> str:
    """Post-process the LLM output to ensure links are present."""
    lines = text.split('\n')
    for i, line in enumerate(lines):
        if '🥑' in line:
            # Look at the previous line for the hotel name
            if i > 0:
                hotel_line = lines[i-1].lower()
                for h_name, link in hotel_link_map.items():
                    # If this hotel name is in the line above the avocado
                    if h_name in hotel_line:
                        # Check if a link already exists nearby (next line)
                        link_exists = False
                        if i + 1 < len(lines) and ('http' in lines[i+1] or 'Посилання відсутнє' in lines[i+1]):
                            link_exists = True
                            # If it's a hallucinated link, we can replace it, but usually LLM just omits it
                            if 'http' in lines[i+1] and link and 'http' in link and link not in lines[i+1]:
                                lines[i+1] = link
                        
                        if not link_exists:
                            # Insert link below the avocado line
...
def _inject_prices(text: str, price_label: str, computed_prices: list) -> str:
    ...
    """
    listed_hotels = _count_listed_hotels(text)
    if listed_hotels > 0:
        computed_prices = computed_prices[:listed_hotels]
...
async def format_tour_message(user_text: str) -> str:
    ...
    for model in models_to_try:
        try:
            ...
            if m:
                data = json.loads(m.group())
                extracted_hotels = data.get("hotels", [])
            if extracted_hotels:
                break
        except Exception as e:
            logger.error(f"Extraction error with {model}: {e}")

    price_data = await extract_prices_from_text(user_text, fast_models)
    logger.info(f"Extracted price data: {price_data}")

    expected_hotels = len(price_data.get("hotel_prices", [])) if price_data else 0
    if candidate_hotels and (
        len(extracted_hotels) < expected_hotels
        or (expected_hotels == 0 and len(extracted_hotels) < min(8, len(candidate_hotels)))
    ):
        ...
        logger.info(
            f"Chunk extraction merged results: initial={len(extracted_hotels)} expected={expected_hotels}"
        )
    ...
    combined = f"ТЕКСТ ВІД МЕНЕДЖЕРА:\n{user_text}\n\nЗНАЙДЕНІ В БАЗІ ГОТЕЛІ:\n{db_text}{prices_block}"
