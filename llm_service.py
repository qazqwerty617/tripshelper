...
def _inject_prices(text: str, enforced_price_line: str) -> str:
    """
...
async def format_tour_message(user_text: str) -> str:
    ...
    prices_block = ""
    enforced_price_line = "💰 не вказано"
    if price_data and price_data.get("hotel_prices") and price_data.get("flight_per_person") is not None:
        ...
        enforced_price_line = f"{price_label} - {prices_str}"
    ...
    for model in models_for_format:
        try:
            ...
            result_text = _inject_prices(result_text, enforced_price_line)
            return result_text
        except Exception as e:
