...
async def format_tour_message(user_text: str) -> str:
    ...
    # deepseek/deepseek-chat is the standard slug for V3 on OpenRouter
    smart_models = [
    ...
    # Improved hint: find the section with hotels and count numbers there
    hotel_count_hint = ""
    hotel_section = ""
    for keyword in ["Готелі:", "готелі:", "в такі готелі:", "варианты:"]:
        if keyword in user_text:
            hotel_section = user_text.split(keyword)[-1]
            break
    
    if hotel_section:
        # Count numbers that look like prices in the hotel section
        prices_in_section = re.findall(r'\b\d{3,4}\b', hotel_section)
        if prices_in_section:
            hotel_count_hint = f"\n\nКІЛЬКІСТЬ ГОТЕЛІВ: {len(prices_in_section)}"
    
    if not hotel_count_hint:
        # Fallback count of all numbers > 50 (approximate)
        all_nums = re.findall(r'\b\d{3,4}\b', user_text)
        if len(all_nums) > 1:
            hotel_count_hint = f"\n\nКІЛЬКІСТЬ ГОТЕЛІВ: {len(all_nums) - 1}" # -1 for flight price
