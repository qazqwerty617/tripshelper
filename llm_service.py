...
4. ФУТЕР: Повідомлення має закінчуватися блоком рекомендацій. Фраза "❗️Ціна актуальна..." йде ПЕРЕД рекомендаціями. НЕ ПОВТОРЮЙ її в самому кінці.
5. ЗІРКИ: Став зірки (3★, 4★, 5★) тільки ОДИН РАЗ поруч із назвою готелю. Якщо в назві вже є зірка, не додавай ще одну.
6. ПОСИЛАННЯ: Пиши ТІЛЬКИ чисте посилання. НЕ додавай слова "Посилання", "Link" або дужки.
"""
...
async def format_tour_message(user_text: str, do_cleanup: bool = False) -> str:
    ...
    if result:
        ...
        # 3. Clean up link suffixes (remove " Посилання]", "[Посилання]" etc.)
        result = re.sub(r'[\s\[(]*Посилання[\]\)]*', '', result, flags=re.IGNORECASE)
        # Remove any lingering brackets or backticks around links
        ...
        # 5. Final pass on double stars and trailing whitespace
        result = re.sub(r'([1-5]★)\s+\1', r'\1', result)
        
        # 6. Remove double footer "❗️Ціна актуальна" if LLM repeated it
        footer_phrase = "❗️Ціна актуальна на момент розрахунку подорожі"
        if result.count(footer_phrase) > 1:
            # Keep only the first occurrence (which is before recommendations)
