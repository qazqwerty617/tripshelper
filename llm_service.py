...
2. КРИТИЧНО: Якщо готелю з тексту НЕМАЄ в наданому списку (навіть приблизно схожого) — НЕ ВИГАДУЙ. У такому випадку поверни оригінальну назву з тексту менеджера, але додай префікс [NOT_FOUND].
...
КРИТИЧНО: Будь ласка, будь дуже уважним до назв. Ти МАЄШ вибрати готель із наданого списку, навіть якщо менеджер вимовив його з сильною помилкою. Якщо збігів нуль — використовуй [NOT_FOUND].
"""
...
def fuzzy_match_hotel(hotel_name: str, db: list) -> tuple[dict, float]:
    ...
    for h in db:
        ...
        if query_brands and db_brands: # Both exist
            ...
            score -= 0.1 # Manager forgot the brand, very light penalty
